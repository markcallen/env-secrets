...
- `env-secrets aws -s <secret-name>` to fetch/inject secret values for a command.
- `env-secrets aws secret <command>` for lifecycle operations (`create`, `update`, `list`, `get`, `delete`).
