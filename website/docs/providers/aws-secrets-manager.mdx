...
- `env-secrets aws secret <command>` for lifecycle operations (`create`, `update`, `copy`, `append`, `remove`, `upsert/import`, `list`, `get`, `value`, `delete`).
...
- `copy` to copy a `SecretString` payload, including JSON secrets, to `--target-region`; `SecretBinary` payloads are not supported
