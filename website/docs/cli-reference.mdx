...
- `--create-config [file]` - Create an env-secrets config file from `--secret` without writing secret keys or values. Defaults to `.env-secrets.yml`; use a `.json` filename for JSON output
...
- `list` - List secrets
- `get` - Get secret metadata/version info (does not print secret values)
- `value` - Get the values of a secret
...
- `value`
  - `-n, --name <name>` (required)
  - `--reveal` — show actual values in table output (default: masked as `****`)
  - `--output <format>` — `json` or `table` (JSON always returns full values; warns on TTY)
  - `-p, --profile <profile>`
  - `-r, --region <region>`
- `delete`
  - `-n, --name <name>` (required)
...
```bash
# Create .env-secrets.yml from an existing secret name
env-secrets aws -s my-app-secrets -r us-east-1 --create-config

# Create JSON config at a custom path
env-secrets aws -s my-app-secrets -r us-east-1 --create-config .env-secrets.json
```

`--create-config` verifies that the secret exists in the resolved region before writing the file. It stores the provider, verified region, explicit profile when provided, and secret names only; it does not write the secret keys or values.
...
# Copy to another region (creates if missing, overwrites if present)
...
# Get secret values (masked by default)
...
# Get values as JSON (always returns full values; warns when stdout is a terminal)
env-secrets aws secret value -n app/dev/api --output json

# Delete with confirmation (recovery window)
