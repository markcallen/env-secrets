...
`--create-config` verifies that the secret exists before writing the file. It stores the provider, effective region/profile when available, and secret names only; it does not write the secret keys or values.
